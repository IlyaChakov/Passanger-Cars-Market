...
df <- read_tsv("Обработанные Данные\\Торговля\\reexport-unit.tsv")

df <- df %>%
    mutate(across(where(is.numeric), ~ ifelse(. < 0, 0, .)))

df <- df %>%
    mutate(across(where(is.numeric), ~ ifelse(df$Страна == "Йемен", NA, .)))

df <- df %>%
    mutate(across(where(is.numeric), ~ ifelse(df$Страна == "Уганда", NA, .)))

df <- df %>%
    mutate(across(where(is.numeric), ~ ifelse(df$Страна == "Эсватини", NA, .)))
...
fwrite(table_top_10, file = "Итоговые Таблицы\\Торговля\\top-reexport-countries.tsv", sep = "\t", quote = FALSE)
