...
forecast_column <- function(column, last_year, h) {
  ts_data <- ts(column, start = last_year - length(column) + 1, frequency = 1)  # Преобразование в временной ряд
