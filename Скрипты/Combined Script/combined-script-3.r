...
process_dataframe <- function(df, start_year, end_year, frequency = 1) {
  ...
  processed_df <- df %>%
    select(-Year) %>%
    summarise(across(everything(), ~ list(process_column(., start_year, end_year, frequency)))) %>%
  ...
  processed_df <- processed_df %>%
    mutate(Year = seq(start_year, end_year))
...
# Применение функции
df_transformed <- process_dataframe(df, start_year, end_year, frequency)
