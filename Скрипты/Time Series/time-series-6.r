...
fill_forecast_ets <- function(data, time = "Date", frequency = 1, cols = "Unit") {
  ...
  # Составляем итоговый датафрейм с сохранением порядка
  final_result <- data %>%
    select(all_of(cols), all_of(time)) %>%
    bind_cols(processed_data) %>%
    select(all_of(original_order)) # Восстанавливаем исходный порядок колонок
...
# Заполнение пропусков и прогнозирование (ежегодные данные)
