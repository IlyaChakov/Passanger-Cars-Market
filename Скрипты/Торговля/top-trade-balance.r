...
df <- read_tsv("Обработанные Данные\\Торговля\\trade-balance-value.tsv")

top_5 <- df %>%
    filter(Год == 2023) %>%
    arrange(Объем) %>%
    slice_max(order_by = Объем, n = 5)

bottom_5 <- df %>%
    filter(Год == 2023) %>%
    arrange(Объем) %>%
    slice_min(order_by = Объем, n = 5)

top_countries <- top_5$Страна
bottom_countries <- bottom_5$Страна

df_top_5 <- df %>%
    filter(Год >= 2010, Страна %in% top_countries) %>%
    group_by(Страна)

df_bottom_5 <- df %>%
    filter(Год >= 2010, Страна %in% bottom_countries) %>%
    group_by(Страна)
...
fwrite(table_merge, file = "Итоговые Таблицы\\Торговля\\top-trade-balance.tsv", sep = "\t", quote = FALSE)
