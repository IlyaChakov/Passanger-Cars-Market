...
df <- read_tsv("Обработанные Данные\\Торговля\\import-unit.tsv")

df <- df %>%
    mutate(across(where(is.numeric), ~ ifelse(. < 0, 0, .)))

df <- df %>%
    mutate(across(where(is.numeric), ~ ifelse(df$Объем > 8, 0, .)))

df <- df %>%
    mutate(across(where(is.numeric), ~ ifelse(df$Страна == "Папуа-Новая Гвинея", NA, .)))

df <- df %>%
    mutate(across(where(is.numeric), ~ ifelse(df$Страна == "Пакистан", NA, .)))

df <- df %>%
    mutate(across(where(is.numeric), ~ ifelse(df$Страна == "Бангладеш", NA, .)))
...
df_top_10 <- df %>%
    filter(Год >= 2010, Страна %in% countries) %>%
    group_by(Страна)
...
fwrite(table_top_10, file = "Итоговые Таблицы\\Торговля\\top-import-countries.tsv", sep = "\t", quote = FALSE)
